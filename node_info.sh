...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about an Evmos node"
		echo
...
validator_address="$evmos_validator_address"
validator_address_variable="evmos_validator_address"
...
main() {
	...
	if [ -n "$wallet_address" ]; then
		...
		local balance=`bc -l <<< "$($daemon query bank balances "$wallet_address" -oj --node "$balance_rpc" 2>/dev/null | jq -r ".balances[0].amount")/1000000000000000000" 2>/dev/null`
	fi
